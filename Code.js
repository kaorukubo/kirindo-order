...
function getMasterData() {
  try {
    ...
    storeProductsRaw.forEach(function (row) {
      ...
      if (storeName && storeOrder.indexOf(storeName) === -1) {
    ...
    /** 販売実績から曜日別参考値を算出 */
    const salesRef = buildSalesReference_();
...
function ensureMasterInitialized_() {
  ...
  if (!ss.getSheetByName(SHEET_SALES)) {
    writeSheet_(SHEET_SALES, SALES_HEADERS, buildSampleSalesRows_(
      getStoreNamesFromSheet_(ss),
      getProductNamesFromSheet_(ss)
    ));
  }
...
function importSalesFromSheet(options) {
  ...
  try {
    ...
    for (var i = 1; i < values.length; i++) {
      ...
      if (String(row[0]).trim() === '日付') continue;

      var dateStr = formatDateCell_(row[0]);
      var store = String(row[1] || '').trim();
      var product = String(row[2] || '').trim();
      var sales = Number(row[3]) || 0;
      var loss = Number(row[4]) || 0;
...
function appendSalesRows_(rows, upsert) {
  ...
  rows.forEach(function (row) {
    ...
    var found = -1;
    for (var r = 1; r < values.length; r++) {
      if (makeSalesKey_(values[r][0], values[r][1], values[r][2]) === key) {
        found = r + 1;
        break;
      }
    }
    if (found > 0) {
      sheet.getRange(found, 1, 1, SALES_HEADERS.length).setValues([row]);
    } else {
      sheet.getRange(sheet.getLastRow() + 1, 1, 1, SALES_HEADERS.length).setValues([row]);
    }
  });
}
